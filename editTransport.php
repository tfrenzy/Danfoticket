...
$genkey = mysql_result($result, $i, "Price");
$source = mysql_result($result, $i, "Source");
$destination = mysql_result($result, $i, "Destination");
$query_ = "UPDATE `routes` SET `Price` = '$_POST[price]' WHERE `BusService` = '$service' AND `Source`= '$source' AND `Destination` = '$destination'";
$result_ = mysql_query($query_);
}
?>
<p><h2> The price for <?php echo $service ?> has been updated!!</h2></p>
<?php	 
 }
 else {
 ?>
...
<h2 class="search-bus">Transport Services</h2>
                    <table align="center">
<tr><td><strong>Bus Service</strong></td><td><strong>Price</strong></td><td><strong>Departure</strong></td><td><strong>Arrival</strong></td><tr>
<?php
$service = mysql_result($result, $i, "BusService");
$genkey = mysql_result($result, $i, "Price");
$source = mysql_result($result, $i, "Source");
$destination = mysql_result($result, $i, "Destination");
?>
<tr><td bgcolor="#D6D6D6"><?php echo $service; ?></td><td bgcolor="#00FFFF"><?php echo $genkey; ?></td><td bgcolor="#D6D6D6"><?php echo $source; ?></td><td bgcolor="#FF9933"><?php echo $destination; ?></td></tr>
</table>	
<label for="textfield"></label>
<input type="text" disabled class="form-control mid-width input-lg text-box" value=<?php echo str_replace(" ", "_", $service);?> name="service" id="textfield">                        
<label for="textfield2"></label>
<input type="text" class="form-control mid-width input-lg text-box" value=<?php echo $genkey;?> name="price" id="textfield2">
<label for="textfield3"></label>
<?php
}
else {
echo "No transport services set up yet!!";	
}
?>
<button type="submit" name="editprice" class="btn btn-primary"><span></span>Edit this Price</button>
</form>   
    <?php } ?>  
                    </div><!-- .col-md-12 -->
