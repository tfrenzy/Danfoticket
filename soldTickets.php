...
$firstdate = $_POST['datepicker'];
$seconddate = $_POST['datepicker1'];
$firstdat = explode("/", $firstdate);
$seconddat = explode("/", $seconddate);
$firstday = $firstdat[2]."-".(int)$firstdat[0]."-".$firstdat[1];
$secondday = $seconddat[2]."-".$seconddat[0]."-".$seconddat[2];
include("connection.inc");
$query = "SELECT * FROM `tickets` WHERE `Time` >= '$firstday' AND `Time` <= '$secondday' ORDER BY `Time`";
$result = mysql_query($query);
if (mysql_num_rows($result) > 0) {
?>
<h2 class="search-bus">Transport Services</h2>
<table align="center">
<tr><td><strong>Full Name</strong></td><td><strong>Service</strong></td><td><strong>Date purchased</strong></td><td><strong>Price</strong></td><tr>
<?php
for ($i = 0; $i < mysql_num_rows($result); $i++) {
$fullname = mysql_result($result, $i, "FullName");
$service = mysql_result($result, $i, "Service");
$price = mysql_result($result, $i, "Price");
$traveldate = mysql_result($result, $i, "Time");
$tprice += $price;
?>
<tr><td bgcolor="#D6D6D6"><?php echo $fullname; ?></td><td bgcolor="#00FFFF"><?php echo $service; ?></td></td><td bgcolor="#00FFFF"><?php echo $traveldate; ?></td><td bgcolor="#D6D6D6"><?php echo $price; ?></tr>
<?php
}
?>
</table>	
<?php
echo "<h3>Total sales is $tprice</h3>";
}
else {
echo "<h3>No sold tickets for the period</h3>";	
}
}
else {
?>
<form class="headerForm row-centered" method="post" action=""  style="background-image:url(images/water-bg.png)">
<h2 class="search-bus" align="center">Search for bus tickets.</h2>
<div class="row form-fields">
<input type="text" class="form-control mid-width datepicker1" id="datepicker" placeholder="From" name="datepicker">
<input type="text" class="form-control mid-width datepicker2" id="datepicker1" placeholder="To" name="datepicker1">
</div>
	    						
					<button type="submit" name="display" class="btn btn-primary"><span></span>Display</button>
	               </form>    
	<?php } ?>
                    </div><!-- .col-md-12 -->
