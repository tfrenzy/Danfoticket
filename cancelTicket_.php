...
$queery = "DELETE FROM `tickets` WHERE `Price` = '$price' AND `GeneratedKey` = '$genkey' AND `FullName` = '$fullname' AND `Email` = '$email'";
$reesult = mysql_query($queery);

$queery_ = "INSERT INTO `cancelled` VALUES('$fullname', '$email', '$genkey', '$price');";
$reesult_ = mysql_query($queery_);
?>
<p><h2> The ticket <?php echo $genkey ?> has been cancelled!! You will be contacted for your refund.</h2></p>
<?php
}
}
 else {
...
if (mysql_num_rows($result) > 0) {
?>
<h2 class="search-bus">Choose Ticket To Cancel</h2>
<table align="center">
...
?>
<tr><td bgcolor="#D6D6D6"><?php echo $service; ?></td><td bgcolor="#00FFFF"><?php echo $genkey; ?></td><td bgcolor="#D6D6D6"><?php echo $source; ?></td><td bgcolor="#FF9933"><?php echo $destination; ?></td><td bgcolor="#00FFFF"><?php echo $traveldate; ?></td</tr>
</table>	
<?php
}
else {
echo "<h3>You have not purchased any tickets!!</h3>";	
}
?>
<button type="submit" name="cancelticket" class="btn btn-primary"><span></span>Cancel this Ticket</button>
</form>   
    <?php } ?> 
                    </div><!-- .col-md-12 -->
